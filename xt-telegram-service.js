...
/**
 * Telegram Service specifically for XT User Bot notifications
 * Uses TELEGRAM_BOT_TOKEN_xt for order notifications
 * Uses TELEGRAM_BOT_TOKEN_BALANCE for low balance warnings
 */
class XtTelegramService {
  constructor() {
    // XT User Bot notifications token
    this.xtBotToken = process.env.TELEGRAM_BOT_TOKEN_xt;
    this.xtChatId = process.env.TELEGRAM_CHAT_ID;
  ...
  isXtBotConfigured() {
    return !!(this.xtBotToken && this.xtChatId);
  }
  ...
  async sendXtNotification(message, options = {}) {
    if (!this.isXtBotConfigured()) {
      console.warn('⚠️ XT Telegram bot not configured. Skipping notification.');
      return { success: false, error: 'XT Telegram bot not configured' };
    }

    try {
      const payload = {
        chat_id: this.xtChatId,
        text: message,
      ...
      const response = await fetch(`https://api.telegram.org/bot${this.xtBotToken}/sendMessage`, {
        method: 'POST',
        headers: {
      ...
      if (data.ok) {
        console.log('✅ XT Telegram notification sent successfully');
        return { success: true, data };
      } else {
        console.error('❌ XT Telegram API error:', data.description);
        return { success: false, error: data.description };
      }
    } catch (error) {
      console.error('❌ Error sending XT Telegram message:', error.message);
  ...
  /**
   * Format and send XT User Bot order notification
   */
  async notifyXtUserBotOrder(data) {
    const {
      botName,
      symbol,
      orderAmount,
    ...
    const gcbBalance = balances?.gcb ? parseFloat(balances.gcb.availableAmount || 0).toFixed(2) : '0.00';
    const usdtBalance = balances?.usdt ? parseFloat(balances.usdt.availableAmount || 0).toFixed(2) : '0.00';

    let message = `<b>🔄 XT User Bot Order ${statusEmoji}</b>\n\n`;
    ...
    return await this.sendXtNotification(message);
  }
  ...
  async checkAndNotifyLowBalance(data) {
    ...
    if (usdtAmount < this.lowBalanceThreshold) {
      const message = `<b>⚠️ LOW BALANCE WARNING (XT)</b>\n\n` +
        `🤖 <b>Bot:</b> ${botName}\n` +
        `💱 <b>Symbol:</b> ${symbol}\n\n` +
        `💰 <b>Current Balance:</b>\n` +
        ...
        `⚠️ Please top up your XT account to ensure the bot can continue trading.\n\n` +
  ...
  /**
   * Send test notification to XT bot
   */
  async sendTestXtNotification() {
    const message = `<b>🧪 XT User Bot Test Notification</b>\n\n` +
      `✅ XT Telegram bot integration is working correctly!\n` +
  ...
  async sendTestBalanceNotification() {
    const message = `<b>🧪 Balance Bot Test Notification (XT)</b>\n\n` +
...
const xtTelegramService = new XtTelegramService();
export default xtTelegramService;
